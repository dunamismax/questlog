...
- Non-judgmental relapse recovery
...
4. Visible progress every day: health, money, streak, stats, and story progression.
5. Social support without shame: accountability should encourage, not punish.
6. Personalization over generic nudges: interventions adapt by trigger patterns and engagement history.
...
- Passive tracking integrations (HealthKit/Google Fit/wearables)
- Smart notification orchestration
- Full analytics and experimentation framework
- Advanced RPG systems (party raids, seasons, economy, prestige)
...
- Bad habit resist tracker
- Craving risk meter
- "If urge now" one-tap rescue actions
- End-of-day review
...
- Event-based habits (triggered by context)
- Frequency goals (X times/week)
- Ceiling goals for bad habits ("max 1/day")
- Grace windows and pause mode
- Streak types:
  - Current streak
  - Best streak
  - Consistency score (rolling 30/90 days)
...
- Trigger heatmap by:
  ...
  - Location bucket
  - Social context
- 3-5 minute intervention toolkit:
  - Guided breathing
  - Grounding sequence
  - Delay timer
  - Substitution action
  - "Message accountability ally" shortcut
...
- 24-hour stabilization challenge
...
- Quest tiers:
  - Micro quests (under 5 minutes)
  - Standard quests (daily/weekly)
  - Boss quests (behavior milestones)
- Loot and crafting:
  - Cosmetic rewards
  - Functional boosts (non-pay-to-win)
- Seasonal progression:
  - 28-day seasons
  - Themed campaigns
  - Season recap
- Prestige/rebirth mode:
  - Keep meta achievements
...
- Program format:
  - 14-day starter
  - 42-day stabilization
  - 90-day mastery
- Daily lesson + action card
- Reflection prompts
...
- Optional live chat rooms/clinics
- Leaderboards:
  - Opt-in only
  - Personal-best and consistency-first variants
...
- Quality-of-life self-rating trend
...
- Safety messaging for crisis language
- Human escalation pathways (where feasible)
- Transparent "not medical advice" boundaries
...
4. 42-day guided quit challenge.
5. Party/accountability buddy MVP.
6. Smart reminders (time + trigger + missed-action aware).
7. Money saved and milestone celebration system.
...
4. Passive tracking integration (HealthKit/Google Fit for eligible habits).
5. Seasonal campaigns and battle pass style progression.
6. Weekly check-in report + recovery risk prediction.
7. In-app community channels with moderation.
...
4. Voice journaling and emotion classification.
5. Coach/counselor marketplace integrations.
6. Guild economies and social crafting systems.
7. Narrative campaign generator based on user archetype.
...
- Craving event logging rate
- Rescue toolkit usage rate
- Program completion rate
- Buddy/party participation rate
...
- Test types:
  - Reminder timing
  - Recovery message framing
  - Reward schedule intensity
  - Daily plan card order
  - Social prompt timing
- Guardrails:
  - No experiments that increase shame language
  - No experiments that hide opt-out controls
...
- Hallucination mitigation:
  - Retrieval from vetted content library
  - Guardrail prompts
...
- Goal-specific micro lessons
- Cultural/localized variants
...
4. Data integrity tests for streak/recovery calculations.
...
- Quest progression and reward transactions remain atomic.
- Party shared quest progress handles concurrency correctly.
...
4. Users can choose solo, buddy, or party accountability.
5. Progress is visible across behavior, health proxies, money saved, and RPG growth.
...
3. Recovery plan wizard after slip.
4. Money saved model + dashboard card.
...
4. Smart reminder scheduler baseline.
...
2. Move completed backlog items out of planned sections (or annotate as implemented where relevant).
3. Reorder and refresh `## 20) Immediate Execution Queue (Next 2 Sprints)`.
4. Update `Last updated` date at the top of this file.
5. Update `README.md` feature descriptions and any setup/deployment notes affected.
6. Update `AGENTS.md` only if agent workflow/rules/conventions changed.
7. If dates or priorities changed, use concrete dates and remove ambiguous timing.
...
5) rigorous measurement and iteration.
